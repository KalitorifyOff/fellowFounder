...
class Space {
  ...
  static void init() {
    ...
    v = EdgeInsets.symmetric(vertical: AppDimensions.space(0.5)); // 0.5

    h1 = EdgeInsets.symmetric(horizontal: AppDimensions.space());
    v1 = EdgeInsets.symmetric(vertical: AppDimensions.space()); // 1
  ...
  static Widget xf([final double no = 1]) =>
      SizedBox(width: AppDimensions.space(no));

  static Widget yf([final double no = 1]) =>
      SizedBox(height: AppDimensions.space(no));

  static EdgeInsets hf([final double no = 1]) =>
      EdgeInsets.symmetric(horizontal: AppDimensions.space(no));

  static EdgeInsets vf([final double no = 1]) =>
      EdgeInsets.symmetric(vertical: AppDimensions.space(no));

  static EdgeInsets all([final double h = 0.5, final double? v]) =>
